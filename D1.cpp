...
public:
    ...
    void mostrarProductos() const {       
        stack<string> copia = pila;       // genera una copia para trabajar sobre ella
        cout << "Productos en el carrito de " << nombreCliente << ": ";
        if (copia.empty()) {
            cout << "(vacío)";
        } else {
            while (!copia.empty()) {      // mientras que no este vacia
                cout << copia.top();      // imprime el nombre del producto superior
                copia.pop();      // elemina el producto superior y pasa al siguiente
                if (!copia.empty()) cout << ", ";
            }
        }
        cout << "\n";
...
public:
     ...
     */
    void agregarCliente(const string& nombre, const CarritoDeCompras& carrito,      
    ...
    void atenderClientes() {
        ...
        while (!cola.empty()) {
            ...
            if (c.discapacidad) cout << " ♿";
            else if (c.adultoMayor) cout << " 👴";
            else if (c.embarazada) cout << " 🤰"; // mostrar las razones de la prioridad
            cout << ANS_RESET << "\n";
 ...
 */
tuple<bool,bool,bool> askPriorityFlags() {
